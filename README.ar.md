...
# XXD Panel 196｜مشهد أشخاص هندسي من الأعلى
...
- `design-only`: تعرض اللوحة الكاملة ترجمة Panel 196 التصميمية فقط، وتبقى الصورة مرجعاً غير ظاهر.
...
بعد التثبيت أعد تشغيل جلسة Agent ثم استدعِ `$xxd-panel-196`. ويمكن إضافة `--global --agent codex --yes` للتثبيت على مستوى المستخدم.

```text
/xxd-panel-196 portrait.jpg --mode top-bottom --size 3:4 --text prompt --locale ar-SA
...
راجع [SKILL.md](SKILL.md) لعقد التشغيل الكامل، ومهايئ التشغيل [بالإنجليزية](references/xxd-panel-196-prompt.en.md) أو [بالصينية](references/xxd-panel-196-prompt.zh-CN.md).
